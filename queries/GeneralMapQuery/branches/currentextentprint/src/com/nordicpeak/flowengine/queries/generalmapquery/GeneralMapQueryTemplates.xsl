	...
	<xsl:template match="ShowQueryForm">
		...
		<div class="query" id="{$queryID}">
			...
			<script type="text/javascript">
				...
				$(document).ready(function(){
					initGeneralMapQuery('<xsl:value-of select="GeneralMapQueryInstance/GeneralMapQuery/queryID" />', '<xsl:value-of select="/Document/requestinfo/contextpath" /><xsl:value-of select="/Document/fullAlias" />', '<xsl:value-of select="/Document/startExtent" />', '<xsl:value-of select="/Document/lmUser" />', false);
				});
