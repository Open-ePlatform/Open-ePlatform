...
function initGeneralMapQuery(queryID, providerURI, startExtent, lmUser, preview) {
	...
	if(!(typeof MapClientInstance === "undefined")) {
		...
		generalMap.beforeMapFeatureAdded = function(e) {
			...
			if (generalMapQueryMinScales[instance.mapID]) {
				...
				if (instance.map.gui.mapPanel.map.getScale() >= generalMapQueryMinScales[instance.mapID]) {
					...
					$("#minscalebtn_" + instance.mapID).click(function(e) {
						...
						mapPanel.map.zoomToScale(generalMapQueryMinScales[instance.mapID] - 1, true);
						generalMap.minZoomLayer.destroyFeatures();
					});
