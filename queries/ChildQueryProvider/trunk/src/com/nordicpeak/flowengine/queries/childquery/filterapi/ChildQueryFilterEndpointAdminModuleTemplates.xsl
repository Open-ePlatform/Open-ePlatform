	...
	<xsl:template match="ShowChildQueryFilterEndpoint">
		...
		<h1 class="clearfix">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<a class="floatright marginleft" href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/delete/{endpointID}" onclick="return confirm('{$i18n.DeleteConfirm}: {ChildQueryFilterEndpoint/name}?');">
