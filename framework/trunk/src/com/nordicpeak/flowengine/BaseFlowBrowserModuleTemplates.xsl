		...
		<div class="errand-menu buttons-in-desktop" data-menu="errand">
		  	...
		  	<ul>
		  		...
		  		<xsl:if test="$flowInstance/Status/isUserDeletable = 'true'">
			  		<li>
			  			<a href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/delete/{$flowInstance/flowInstanceID}" onclick="return confirmHyperlinkPost(this);" title="{$i18n.DeleteFlowInstanceConfirm}: {$flowInstance/Flow/name}" class="btn btn-red">
