...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="validationError[messageKey='FlowNotAvailiableInRequestedFormat']">
		...
		<xsl:call-template name="printValidationError">
			<xsl:with-param name="message" select="$i18n.FlowNotAvailiableInRequestedFormat"></xsl:with-param>
		</xsl:call-template>
		...
		<div class="errand-menu buttons-in-desktop" data-menu="errand">
		  	...
		  	<ul>
		  		...
		  		<xsl:if test="$flowInstance/Status/isUserDeletable = 'true'">
			  		<li>
			  			<a href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/delete/{$flowInstance/flowInstanceID}" onclick="return confirmPost(this);" title="{$i18n.DeleteFlowInstanceConfirm}: {$flowInstance/Flow/name}" class="btn btn-red">
			...
			<div class="errand-menu buttons-in-desktop" data-menu="errand">
			  	...
			  	<ul>
			  		...
			  		<xsl:if test="$flowInstance/Status/isAdminDeletable = 'true' or $deleteAccessOverride">
				  		<li>
				  			<a href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/delete/{$flowInstance/flowInstanceID}" onclick="return confirmPost(this);" title="{$i18n.DeleteFlowInstanceConfirm}: {$flowInstance/Flow/name}" class="btn btn-light">
