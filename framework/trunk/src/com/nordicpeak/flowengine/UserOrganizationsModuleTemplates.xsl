	...
	<xsl:template match="Organization" mode="list">
		...
		<tr>
			...
			<td class="link">
				...
				<a class="btn btn-red vertical-align-middle" style="margin-left: 2px" href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/delete/{organizationID}" onclick="return confirmHyperlinkPost(this);" title="{$i18n.DeleteOrganizationConfirm}: {name}"><xsl:value-of select="$i18n.Delete" /></a>
			</td>
		</tr>
		...
		<article class="clearfix">
			...
			<fieldset>
				...
				<div>
					...
					<xsl:call-template name="createCheckbox">
						...
						<xsl:with-param name="checked">
							<xsl:if test="$organization/email != ''">true</xsl:if>
						</xsl:with-param>
					</xsl:call-template>
					<label for="contactByEmail" class="checkbox disabled"><xsl:value-of select="$i18n.AllowContactByEmail" /></label><br/>
				...
				<div>
					...
					<label for="contactBySMS" class="checkbox"><xsl:value-of select="$i18n.AllowContactBySMS" /></label><br/>
