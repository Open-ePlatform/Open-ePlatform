...
public class ContactDetailQueryCRUD extends BaseQueryCRUD<ContactDetailQuery, ContactDetailQueryProviderModule> {
	...
	@Override
	protected ContactDetailQuery populateFromUpdateRequest(ContactDetailQuery oldQuery, HttpServletRequest req, User user, URIParser uriParser) throws ValidationException, Exception {
		
		ContactDetailQuery query = super.populateFromUpdateRequest(oldQuery, req, user, uriParser);
		
		List<ValidationError> validationErrors = new ArrayList<ValidationError>();
		
		populateQueryDescriptor((QueryDescriptor) query.getQueryDescriptor(), req, validationErrors);
		
		if (query.getFieldEmail() == ContactDetailQueryField.HIDDEN && query.getFieldMobilePhone() != ContactDetailQueryField.HIDDEN) {
			
			query.setAllowSMSNotification(true);
		}
