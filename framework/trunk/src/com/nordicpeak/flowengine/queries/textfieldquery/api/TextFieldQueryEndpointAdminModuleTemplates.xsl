	...
	<xsl:template match="ShowTextFieldQueryEndpoint">
		...
		<h1 class="clearfix">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<a class="floatright marginleft" href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/delete/{endpointID}" onclick="return confirmPost(this);" title="{$i18n.DeleteConfirm}: {name}">
