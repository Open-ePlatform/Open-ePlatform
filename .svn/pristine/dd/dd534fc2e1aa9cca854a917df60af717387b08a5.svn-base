...
function initCheckBoxQuery(queryID) {
	...
	if($freeTextAlternative.length > 0) { 
		...
		$freeTextAlternative.on("change", function(e, data) {
			...
			const $freeTextAlternativeValue = $("#" + $this.attr("name") + "Value");
			
			if($this.is(":checked")) {
				
				if (!$freeTextAlternativeValue.parent().hasClass("forceDisabled")) {
					$freeTextAlternativeValue.removeAttr("disabled");
				}
				
				$freeTextAlternativeValue.parent().show();
			} else {
				$freeTextAlternativeValue.attr("disabled", "disabled").parent().hide();
			}
