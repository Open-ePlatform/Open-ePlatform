...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="ExternalMessage" mode="admin">
		...
		<xsl:call-template name="createAdminMessage">
			...
			<xsl:with-param name="isInternalMessage" select="false()"/>
	...
	<xsl:template match="InternalMessage" mode="admin">
		...
		<xsl:call-template name="createAdminMessage">
			<xsl:with-param name="message" select="." />
			<xsl:with-param name="isInternalMessage" select="true()"/>
	...
	<xsl:template name="createAdminMessage">
		...
		<xsl:param name="isInternalMessage" />
		...
		<xsl:variable name="isMessageDeleted" select="$message/deleted"/>
		...
		<li id="{$prefix}-{$message/messageID}" class="official" data-message-id="{$message/messageID}">
			...
			<xsl:attribute name="class">
				...
				<xsl:if test="$isMessageDeleted"> deleted </xsl:if>
			...
			<div class="message" data-messageid="{$message/messageID}">
				...
				<div class="d-flex justify-content-between">
					<div>
						<xsl:choose>
							<xsl:when test="$isMessageDeleted">
								<xsl:value-of select="$i18n.Message.Deleted"/>
							</xsl:when>
					...
					<xsl:if test="$isInternalMessage and ../../Flow/allowDeletingInternalMessages = 'true' and not($isMessageDeleted)">
						<div>
							<button class="btn btn-red delete-btn bigmarginright" onclick="return confirmPost('{/Document/requestinfo/uri}?internalmessage-delete={$message/messageID}','{$i18n.DeleteConfirmation}')">
				...
				<xsl:variable name="isMessageDeletedByPoster" select="$isMessageDeleted and $message/poster/userID = $message/deletedBy/userID"/>
				
				<div class="d-flex flex-column justify-content-center">
					<div class="author">
						<i data-icon-before="m" aria-hidden="true"/>
						...
						<xsl:call-template name="printUser">
					 	...
					 	<span class="time">
					 		...
					 		<xsl:text>: </xsl:text>
					 		<xsl:value-of select="$message/added" />
					 	</span>
					 	
					 	<xsl:if test="$isMessageDeletedByPoster">
					 		<span class="time">
								<xsl:text>&#160;·&#160;</xsl:text>
								<xsl:value-of select="$i18n.Deleted"/>
								<xsl:text>: </xsl:text>					 		
						 		<xsl:value-of select="$message/deleted" />
					 		</span>
					 	</xsl:if>
					 </div>
					 
					<xsl:if test="$isMessageDeleted and not($isMessageDeletedByPoster)">
						<div class="author">
							<i data-icon-before="t" aria-hidden="true"/>
							
						 	<xsl:call-template name="printUser">
						 		<xsl:with-param name="user" select="$message/deletedBy"/>
						 	</xsl:call-template>
