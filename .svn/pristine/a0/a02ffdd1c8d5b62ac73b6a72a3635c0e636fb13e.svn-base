...
public class FlowInstanceAttachmentsModule extends AnnotatedForegroundModule implements ViewFragmentModule<ForegroundModuleDescriptor>, FlowAdminFragmentExtensionViewProvider {
	...
	@ModuleSetting
	@TextFieldSettingDescriptor(name = "New attachments email subject (managers)", description = "The subject of emails sent to the managers when new attachments are added", required =  true)
	@XSLVariable(prefix = "java.")
	private String newAttachmentsManagerEmailSubject;
	
	@ModuleSetting
	@HTMLEditorSettingDescriptor(name = "New attachments email message (managers)", description = "The message of emails sent to the managers when new attachments are added", required = true)
	@XSLVariable(prefix = "java.")
	private String newAttachmentsManagerEmailMessage;
	...
	public ForegroundModuleResponse processOverviewExtensionRequest(FlowInstance flowInstance, HttpServletRequest req, HttpServletResponse res, URIParser uriParser, User user, boolean manager) throws Exception {
		...
		if (manager || settings.isAllowUserUpload()) {
			...
			if (MultipartRequest.isMultipartRequest(req)) {
				...
				addAttachment(req, res, user, uriParser, flowInstance, manager);
				return null;
	...
	public ViewFragment getTabContentsViewFragment(FlowInstance flowInstance, HttpServletRequest req, URIParser uriParser, List<ValidationError> validationErrors, boolean manager) throws SQLException, TransformerException {
		...
		if (manager) {
			...
			if (settings.isAllowUserUpload()) {
				XMLUtils.appendNewElement(doc, tabElement, "AllowUserUpload");
			}
	...
	public void addAttachment(HttpServletRequest req, HttpServletResponse res, User user, URIParser uriParser, FlowInstance flowInstance, boolean manager) throws SQLException, IOException, TransformerException {
			...
			try {
				...
				if (!CollectionUtils.isEmpty(attachments)) {
					...
					if (!CollectionUtils.isEmpty(flowInstance.getOwners()) && settings != null && settings.isModuleEnabled()) {
						...
						if (contacts != null) {
							...
							for (Contact contact : contacts) {
								...
								if (manager) {
									...
									if (settings.isUserEmailEnabled()) {
									...
									if (settings.isUserSmsEnabled()) {
										flowNotificationHandler.sendContactSMS(flowInstance, contact, newAttachmentsUserSMS, forceSMS);
									}
						...
						if (settings.isManagerEmailEnabled()) {
							List<User> managers = Optional.ofNullable(flowInstance.getManagers()).orElse(Collections.emptyList());
							List<User> excludedManagers = new ArrayList<>(1);
							
							for (User instanceManager : managers) {
								boolean isPosterManager = user.equals(instanceManager);
								
								if (isPosterManager) {
									excludedManagers.add(instanceManager);
									break;
								}
							}
							
							flowNotificationHandler.sendManagerEmails(flowInstance, null, newAttachmentsManagerEmailSubject, attachmentsUpdatedMessage, excludedManagers, false);
						}
					}
		...
		if (validationErrors.isEmpty()) {
			...
			jsonResponse.putField("response", getTabContentsViewFragment(flowInstance, req, uriParser, validationErrors, manager).getHTML());
	...
	private ForegroundModuleResponse showUpdateSettings(String extensionRequestURL, HttpServletRequest req, HttpServletResponse res, User user, URIParser uriParser) throws Exception {
		...
		if (settings == null) {
			...
			settings.setUserEmailEnabled(false);
			settings.setUserSmsEnabled(false);
		}
	...
	private void updateSettings(HttpServletRequest req, Flow flow) throws SQLException {
		...
		boolean moduleEnabled = BooleanUtils.toBoolean(req.getParameter("moduleEnabled"));
		boolean allowUserUpload = BooleanUtils.toBoolean(req.getParameter("allowUserUpload"));
		boolean userEmailEnabled = BooleanUtils.toBoolean(req.getParameter("userEmailEnabled"));
		boolean userSmsEnabled = BooleanUtils.toBoolean(req.getParameter("userSmsEnabled"));
		boolean managerEmailEnabled = BooleanUtils.toBoolean(req.getParameter("managerEmailEnabled"));

		FlowInstanceAttachmentsSettings settings = attachmentSettingsDAOWrapper.get(flow.getFlowFamily().getFlowFamilyID());

		if (settings == null) {
			settings = new FlowInstanceAttachmentsSettings();
			populateSettings(flow, moduleEnabled, userEmailEnabled, userSmsEnabled, managerEmailEnabled, allowUserUpload, settings);
	...
	private void populateSettings(Flow flow, boolean moduleEnabled, boolean emailEnabled, boolean smsEnabled, boolean managerEmailEnabled, boolean allowUserUpload, FlowInstanceAttachmentsSettings settings) {

		settings.setFlowFamilyID(flow.getFlowFamily().getFlowFamilyID());
		settings.setModuleEnabled(moduleEnabled);
		settings.setAllowUserUpload(allowUserUpload);
		
		settings.setUserEmailEnabled(emailEnabled);
		settings.setUserSmsEnabled(smsEnabled);
		
		settings.setManagerEmailEnabled(managerEmailEnabled);
	}
