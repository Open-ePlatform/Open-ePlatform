			...
			<form method="GET" action="{/Document/requestinfo/uri}">
				...
				<div class="service-navigator-wrap">
					<div>
						...
						<ol class="service-navigator primary">
	...
	<xsl:template match="MultiSigningStatusForm">
		...
		<xsl:if test="SubmitSurveyHTML">
			<div class="info-box success">
				<span><strong><xsl:value-of select="$i18n.SubmitSurveyTitle" /></strong><i class="icon close" aria-hidden="true">x</i></span>
...
<!-- 				</div> -->
			...
			<div class="section-full padtop receipt">
				...
				<div>
					...
					<span data-icon-before="c">
						...
						<i class="icon close" aria-hidden="true">x</i>
					</span>
					<div class="marker"></div>
				</div>
				...
				<div>
  				...
  				<div class="heading-wrapper">
	  				...
	  				<div class="errand-header">
  						...
  						<xsl:if test="FlowInstance/Flow/hideOverviewFlowInstance = 'false'">
  						...
  						<xsl:if test="FlowInstance/Flow/hideOverviewPDF = 'false'">
	  						<xsl:choose>
			...
			<form method="post" action="{/Document/requestinfo/uri}" enctype="multipart/form-data">
				...
				<div class="service-navigator-wrap">
					<div>
						...
						<ol class="service-navigator primary" aria-label="{$i18n.ListOfSteps}">
							<xsl:apply-templates select="FlowInstance/Flow/Steps"/>
						</ol>
						<a data-icon-after="&gt;" href="#" class="js-next" aria-label="{$i18n.Next}">
		  					<span><xsl:value-of select="$i18n.Next" /></span>
				...
				<div class="section-full push">
					...
					<div class="queries">
						...
						<xsl:if test="not(FlowInstance/Flow/hideAsteriskInfo = 'true')">			
							<div class="info"><p><span class="info-symbol" aria-hidden="true"></span><xsl:value-of select="$i18n.AsteriskInfo"/></p></div>
						</xsl:if>
					...
					<div class="navigator-buttons">
						<xsl:if test="ManagerResponse/currentStepIndex > 0">
								...
								<xsl:variable name="aria-label-back">
									<xsl:value-of select="$i18n.GoToPreviousStep"></xsl:value-of>
									<xsl:value-of select="FlowInstance/Flow/Steps/Step[position() = $currentStepIndex]/name"></xsl:value-of>		
								</xsl:variable>
								
								<a href="#" class="btn btn-light xl prev arrow-mobile force-submit" onclick="submitStep('back', event)" data-icon-before="&#60;" aria-label="{$aria-label-back}"><span class="only-mobile"><xsl:value-of select="$i18n.previousStep" /></span><span class="hide-mobile"><xsl:value-of select="FlowInstance/Flow/Steps/Step[position() = $currentStepIndex]/name" /></span></a>
						...
						<xsl:choose>
							<xsl:when test="$currentStepID = FlowInstance/Flow/Steps/Step[last()]/stepID">
								...
								<xsl:choose>
									<xsl:when test="FlowInstance/Flow/usePreview = 'true'">
										...
										<xsl:variable name="aria-label-preview">
											<xsl:value-of select="$i18n.GoToNextStep"></xsl:value-of>
											<xsl:value-of select="$i18n.preview" />		
										</xsl:variable>
									
										<a href="#" class="btn btn-green xl next arrow-mobile force-submit" onclick="submitStep('preview', event)" data-icon-after=">" aria-label="{$aria-label-preview}"><span class="only-mobile"><xsl:value-of select="$i18n.nextStep" /></span><span class="hide-mobile"><xsl:value-of select="$i18n.preview" /></span></a>
									</xsl:when>
									<xsl:otherwise>
							...
							<xsl:otherwise>
									...
									<xsl:variable name="aria-label-forward">
										<xsl:value-of select="$i18n.GoToNextStep"></xsl:value-of>
										<xsl:value-of select="FlowInstance/Flow/Steps/Step[position() = ($currentStepIndex + 2)]/name"></xsl:value-of>		
									</xsl:variable>
									
									<xsl:value-of select="currentStepIndex" />
									<a href="#" class="btn btn-green xl next arrow-mobile force-submit" onclick="submitStep('forward', event)" data-icon-after=">" aria-label="{$aria-label-forward}"><span class="only-mobile"><xsl:value-of select="$i18n.nextStep" /></span><span class="hide-mobile"><xsl:value-of select="FlowInstance/Flow/Steps/Step[position() = ($currentStepIndex + 2)]/name" /></span></a>
							</xsl:otherwise>
						</xsl:choose>
					</div>
		...
		<xsl:if test="../usePreview = 'true'">
			<li>
				<xsl:attribute name="aria-label">
					<xsl:value-of select="$i18n.Step"></xsl:value-of>
					<xsl:value-of select="$stepCount + 1"></xsl:value-of>	
					<xsl:text> </xsl:text>
					<xsl:value-of select="$i18n.preview" />				
				</xsl:attribute>
		...
		<xsl:choose>
			<xsl:when test="../paymentSupportEnabled = 'true' and ../requireSigning = 'true' and not(../skipPosterSigning = 'true')">
				...
				<li>
					<xsl:attribute name="aria-label">
						<xsl:value-of select="$i18n.Step"></xsl:value-of>
						<xsl:value-of select="$stepCount + 2 + $previewOffset"></xsl:value-of>	
						<xsl:text> </xsl:text>
			...
			<xsl:when test="../paymentSupportEnabled = 'true'">
				...
				<li>
					<xsl:attribute name="aria-label">
						<xsl:value-of select="$i18n.Step"></xsl:value-of>
						<xsl:value-of select="$stepCount + 1 + $previewOffset"></xsl:value-of>	
						<xsl:text> </xsl:text>
						<xsl:value-of select="$i18n.payAndSubmit" />				
					</xsl:attribute>
	...
	<xsl:template name="createSubmitStep">
		...
		<li>
			<xsl:attribute name="aria-label">
				<xsl:value-of select="$i18n.Step"></xsl:value-of>
				<xsl:value-of select="$step"></xsl:value-of>	
				<xsl:text> </xsl:text>
				<xsl:value-of select="$submitText" />				
			</xsl:attribute>
	...
	<xsl:template match="Step">
		...
		<li>
			...
			<xsl:choose>
				<xsl:when test="$currentStepID = stepID">
					<xsl:attribute name="class">active</xsl:attribute><xsl:attribute name="aria-current">step</xsl:attribute>
				</xsl:when>
