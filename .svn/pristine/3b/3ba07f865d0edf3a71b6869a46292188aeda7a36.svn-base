...
public class ContactDetailQueryProviderModule extends BaseQueryProviderModule<ContactDetailQueryInstance> implements BaseQueryCRUDCallback {
	...
	public void populate(ContactDetailQueryInstance queryInstance, HttpServletRequest req, User user, User poster, boolean allowPartialPopulation, MutableAttributeHandler attributeHandler, InstanceRequestMetadata requestMetadata) throws ValidationException {
		...
		if (query.isRequireAtLeastOneContactWay() && query.isAllowSMSNotification() && query.getFieldMobilePhone() != ContactDetailQueryField.HIDDEN && query.getFieldEmail() == ContactDetailQueryField.VISIBLE && !allowPartialPopulation) {
			...
			if ((mobilePhoneEmpty && emailEmpty) || (emailEmpty && !mobilePhoneEmpty && !contactBySMS) || (fieldsRequired && !contactBySMS && emailEmpty)) {
				
				validationErrors.add(new ValidationError("NoContactChannelChoosen", qParam + "notification-channel-settings", qParam));
			}
		}
		
		if (query.isRequireMobilePhoneOrPhone() && !allowPartialPopulation) {

			if (mobilePhoneEmpty && phoneEmpty) {
				
				validationErrors.add(new ValidationError("RequireMobilePhoneOrPhone", qParam));

			}
		}
		
		if (query.isRequireContactDetailInfoVerification()) {
			String confirmEmailID = qParam + "confirmEmail";
			String confirmMobilePhoneID = qParam + "confirmMobilePhone";

			String confirmEmail = ValidationUtils.validateParameter(confirmEmailID, req, query.isRequireContactDetailInfoVerification() && query.getFieldEmail() != ContactDetailQueryField.HIDDEN && !allowPartialPopulation && !emailEmpty, 0, 255, EmailPopulator.getPopulator(), validationErrors);
			String confirmMobilePhone = ValidationUtils.validateParameter(confirmMobilePhoneID, req, query.isRequireContactDetailInfoVerification() && query.getFieldMobilePhone() != ContactDetailQueryField.HIDDEN && !allowPartialPopulation && !mobilePhoneEmpty, StringSwedishPhoneNumberPopulator.getPopulator(), validationErrors);
			
			if (((email == null && confirmEmail != null) || (email != null && !email.equalsIgnoreCase(confirmEmail))) && !ValidationUtils.containsValidationErrorForField(confirmEmailID, validationErrors)) {

				validationErrors.add(new ValidationError("EmailVerificationMismatch", "", confirmEmailID));
			}
			
			if (mobilePhone !=null && confirmMobilePhone != null && query.getFieldMobilePhone() != ContactDetailQueryField.HIDDEN) {
				...
				if ((mobilePhone != null && !strippedMobilePhone.equals(strippedConfirmMobilePhone)) && !ValidationUtils.containsValidationErrorForField(confirmMobilePhoneID, validationErrors)) {
					validationErrors.add(new ValidationError("MobilePhoneVerificationMismatch", "", confirmMobilePhoneID));
				}
			} else if(mobilePhoneEmpty && confirmMobilePhone != null) {
				validationErrors.add(new ValidationError("MobilePhoneVerificationMismatch", "", confirmMobilePhoneID));
			}		
		}
