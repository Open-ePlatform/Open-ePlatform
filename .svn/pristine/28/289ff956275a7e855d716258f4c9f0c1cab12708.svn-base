...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
			...
			<article>
				...
				<div class="heading-wrapper">
					...
					<xsl:call-template name="createUpdateButton">
						...
						<xsl:with-param name="queryName" select="PersonDataInformerQueryInstance/QueryInstanceDescriptor/QueryDescriptor/name" />
					</xsl:call-template>
				...
				<xsl:choose>
					<xsl:when test="PersonDataInformerQueryInstance/ownerName">
						<p>
							<strong>
					...
					<xsl:when test="PersonDataInformerQueryInstance/FlowFamily/ownerName">
						<p>
							<strong>
								<xsl:value-of select="$i18n.Accountable"/>
							</strong>
	...
	<xsl:template match="ShowQueryForm">
		...
		<div class="query persondatainformerquery" id="{$queryID}">
			...
			<article>
				...
				<xsl:choose>
					<xsl:when test="PersonDataInformerQueryInstance/isInvalidConfiguration = 'true'">
						<p class="error">
							<xsl:value-of select="$i18n.Error.InvalidConfiguration"/>
						</p>
					</xsl:when>
	...
	<xsl:template match="FlowFamilyInformerSetting">
		...
		<p>
			...
			<ul class="description-text">
				<xsl:choose>
					<xsl:when test="../../SelectedDataAlternatives">
						<xsl:apply-templates select="../../SelectedDataAlternatives/selectedDataAlternative" mode="show"/>
					</xsl:when>
					<xsl:otherwise>
		...
		<p>
			...
			<ul class="description-text">
				<xsl:choose>
					<xsl:when test="../../SelectedReasonAlternatives">
						<xsl:apply-templates select="../../SelectedReasonAlternatives/selectedReasonAlternative" mode="show"/>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template match="selectedDataAlternative" mode="show">
	...
	<xsl:template match="selectedReasonAlternative" mode="show">
