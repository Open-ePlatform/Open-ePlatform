...
public class FlowFamilyUtils {
	...
	public static List<Group> getAllowedManagerGroups(ImmutableFlowFamily flowFamily, GroupHandler groupHandler) {
		...
		return getAllowedManagerGroups(flowFamily, groupHandler, false);
	}

	public static List<Group> getAllowedManagerGroups(ImmutableFlowFamily flowFamily, GroupHandler groupHandler, boolean checkHiddenAssignment) {

		if (flowFamily != null && !CollectionUtils.isEmpty(flowFamily.getManagerGroups())) {

			List<Integer> groupIDs = new ArrayList<>(flowFamily.getManagerGroups().size());

			for (FlowFamilyManagerGroup managerGroup : flowFamily.getManagerGroups()) {

				if ((managerGroup.isRestricted() && !checkHiddenAssignment) || (managerGroup.isRestricted() && checkHiddenAssignment && !managerGroup.isHideGroupInAssignment())) {
