			...
			if (apiModule == null) {
				...
				if (queryInstance.getLastUsedEndpointURL() == null || !queryInstance.getLastUsedEndpointURL().equals(endpointURL)) {
					...
					try {
						...
						if(query.getEndpoint() != null && query.getEndpoint().isResetQueryOnAPIException()) {
							
							queryInstance.reset((MutableAttributeHandler) attributeHandler);
						}
					}
		...
		for (TextField textField : textFields) {
			...
			if (textField.isDisabled()) {
				...
				if (fieldValue != null) {
					...
					if (fieldValue.getValue().length() > 255) {
						validationErrors.add(new ValidationError("LockedFieldValueTooLong", "", String.valueOf(textField.getTextFieldID())));
					} else {
