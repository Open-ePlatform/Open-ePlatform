...
public class DropDownQueryProviderModule extends BaseQueryProviderModule<DropDownQueryInstance> implements BaseQueryCRUDCallback {
	...
	@Override
	public void save(DropDownQueryInstance queryInstance, TransactionHandler transactionHandler, InstanceRequestMetadata requestMetadata) throws Throwable {
	...
	@Override
	public void populate(DropDownQueryInstance queryInstance, HttpServletRequest req, User user, User poster, boolean allowPartialPopulation, MutableAttributeHandler attributeHandler, InstanceRequestMetadata requestMetadata) throws ValidationException {
		...
		Integer alternativeID = NumberUtils.toInt(req.getParameter("q" + queryID + "_dropdownalternative"));
		...
		if (queryInstance.getQuery().getFreeTextAlternative() != null && !alternativeSelected) {
			...
			freeTextAlternativeValue = FreeTextAlternativePopulator.populate(queryID, "_dropdownalternative", req, validationErrors);
		...
		if (queryInstance.getQueryInstanceDescriptor().getQueryState() == QueryState.VISIBLE_REQUIRED && !alternativeSelected) {
			...
			validationErrors.add(new ValidationError("RequiredQuery", "", "q" + queryInstance.getQuery().getQueryID() + "_dropdownalternative"));
		}
	...
	private void validateFreeTextValue(String freeTextAlternativeValue, DropDownQueryInstance queryInstance, List<ValidationError> validationErrors) {
		...
		if (queryInstance.getQuery().getFormatValidator() != null) {
			...
			if (formatValidator == null) {
				...
				validationErrors.add(new InvalidFormatValidationError(queryInstance.getQuery().getQueryID() + "_dropdownalternativeValue", queryInstance.getQuery().getInvalidFormatMessage()));
			}
		}
	}

	private StringFormatValidator getStringFormatValidator(DropDownQuery query) {
