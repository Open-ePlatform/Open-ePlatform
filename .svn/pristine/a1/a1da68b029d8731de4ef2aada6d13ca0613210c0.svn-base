...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
			...
			<article>
				...
				<div class="heading-wrapper">
					...
					<xsl:call-template name="createUpdateButton">
						...
						<xsl:with-param name="queryName" select="TextFieldQueryInstance/QueryInstanceDescriptor/QueryDescriptor/name" />
					</xsl:call-template>
	...
	<xsl:template match="ShowQueryForm">
		...
		<xsl:variable name="locked" select="(TextFieldQueryInstance/TextFieldQuery/lockForManagerUpdate = 'true' and RequestMetadata/manager = 'true') or TextFieldQueryInstance/locked = 'true'" />
		...
		<div class="query textfieldquery" id="{$queryID}">
			...
			<div class="validationErrorTemplate hidden">
				<xsl:call-template name="fieldValidationErrorTemplate"/>
			</div>
			...
			<xsl:if test="ValidationErrors/validationError">
				...
				<div id="{$queryID}-validationerrors" class="validationerrors">
					<div class="info-box error">
						...
						<xsl:variable name="validationErrors" select="ValidationErrors/validationError[messageKey and not(messageKey = 'LockedFieldValueTooLong')]"/>
			...
			<article>
				...
				<xsl:if test="TextFieldQueryInstance/lockedMessage">
					<p class="full">
						<xsl:value-of select="TextFieldQueryInstance/lockedMessage"/>
					</p>
				</xsl:if>
	...
	<xsl:template match="TextField">
		...
		<xsl:variable name="validationError" select="../../../../ValidationErrors/validationError[fieldName = $textFieldID]"/>
		...
		<div class="split break {$class}">
			...
			<xsl:call-template name="createTextField">
				...
				<xsl:with-param name="disableAutocomplete"><xsl:if test="inactivateAutocomplete = 'true'"><xsl:text>true</xsl:text></xsl:if></xsl:with-param>
	...
	<xsl:template match="validationError[validationErrorType = 'RequiredField']">
		...
		<xsl:call-template name="fieldValidationErrorTemplate">
			<xsl:with-param name="id" select="$id"/>
			<xsl:with-param name="message" select="$i18n.RequiredField"/>
		</xsl:call-template>
	...
	<xsl:template match="validationError[messageKey = 'LockedFieldValueTooLong']">
		
		<xsl:param name="id" />
		
		<xsl:call-template name="fieldValidationErrorTemplate">
			<xsl:with-param name="id" select="$id"/>
			<xsl:with-param name="message" select="$i18n.LockedFieldValueTooLong"/>
		</xsl:call-template>
	...
	<xsl:template match="validationError">
		...
		<xsl:call-template name="fieldValidationErrorTemplate">
			<xsl:with-param name="id" select="$id"/>
			<xsl:with-param name="message" select="$i18n.UnknownValidationError"/>
		</xsl:call-template>
		
	</xsl:template>
	
	<xsl:template name="fieldValidationErrorTemplate">
	
		<xsl:param name="id" select="null"/>
		<xsl:param name="message" select="null" />
