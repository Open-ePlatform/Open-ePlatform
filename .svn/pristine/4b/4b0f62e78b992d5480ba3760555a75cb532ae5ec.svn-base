...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="exsl">
								...
								<div class="help-box" data-help-box="askLogin" aria-modal="true" role="dialog">
									<div>
							  			<div> 
							  				...
							  				<xsl:variable name="close-helpbox-info">
							  					<xsl:value-of select="$i18n.CloseBox"></xsl:value-of>
							  					<xsl:value-of select="$i18n.StartFlow.LoginQuestion.title"></xsl:value-of>
							  				</xsl:variable>
							  				<a class="close btn" href="#" role="button" data-icon-after="x" aria-label="{$close-helpbox-info}"><xsl:value-of select="$i18n.Close"></xsl:value-of></a> 
	...
	<xsl:template name="appendFlowContactAndOwner">
		...
		<xsl:if test="$ownerExtensionView">
			...
			<div class="about-flow-extension full border border-radius-small noexpander simplebox">

				<div class="simplebox-title" role="button" tabindex="0" aria-expanded="false"><xsl:value-of select="$ownerExtensionView/name" /></div>

				<div class="simplebox-content" style="display: none">
			...
			<xsl:if test="FlowFamily/contactName">
				...
				<div class="about-flow-extension full border border-radius-small noexpander simplebox">
					...
					<div class="simplebox-title" role="button" tabindex="0" aria-expanded="false"><xsl:value-of select="$i18n.Questions" /></div>
			...
			<div class="btn-wrapper no-border">
				...
				<xsl:choose>
					<xsl:when test="$isDisabled and not($operatingMessage/allowFlowForms = 'true')">
						...
						<a class="btn btn-blue xl disabled full flowFormDownload" href="javascript:void(0)" title="{$operatingMessage/message}">
							<xsl:choose>
					...
					<xsl:when test="count($flow/FlowForms/FlowForm) = 1">
						...
						<xsl:variable name="formFileSize">
							<xsl:value-of select="$flow/FlowForms/FlowForm/formattedSize" />
						</xsl:variable>
					
						<xsl:variable name="title">
							<xsl:value-of select="$i18n.DownloadFlowFormDescription.Part1" />
								<xsl:text>&#160;</xsl:text>
								<xsl:choose>
									<xsl:when test="$flow/FlowForms/FlowForm/name"><xsl:value-of select="$flow/FlowForms/FlowForm/name" /></xsl:when>
									<xsl:otherwise><xsl:value-of select="$flow/name" /></xsl:otherwise>
								</xsl:choose>
								<xsl:text>&#160;</xsl:text>
								<xsl:value-of select="$i18n.DownloadFlowFormDescription.Part2" />
								<xsl:text>&#160;</xsl:text>
								<xsl:value-of select="$flow/FlowForms/FlowForm/fileExtension" />-<xsl:value-of select="$i18n.DownloadFlowFormDescription.Part3" />
								<xsl:if test="$formFileSize != ''">
									<xsl:text>, </xsl:text>
									<xsl:value-of select="$formFileSize"/>							
								</xsl:if>
						</xsl:variable>
					
						<a class="btn btn-blue xl full flowFormDownload" href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/getflowform/{$flow/flowID}/{$flow/FlowForms/FlowForm[1]/flowFormID}" target="_blank">
							...
							<xsl:variable name="formFileInfo">
							...
							<xsl:if test="$formFileSize != ''">
								<xsl:value-of select="$formFileInfo"/>
							</xsl:if>
					...
					<xsl:otherwise>
						...
						<a id="flowforms-list-button" role="button" class="btn btn-blue xl full" href="#" title="{$i18n.DownloadFlowFormsDescription} {$flow/name}" aria-label="{$i18n.DownloadFlowFormsDescription} {$flow/name}" aria-expanded="false">
							<xsl:choose>
								<xsl:when test="$flow/FlowForms/FlowForm/downloadFormButtonText">
									<xsl:value-of select="$flow/FlowForms/FlowForm/downloadFormButtonText"/>
								</xsl:when>
		...
		<a class="display-block text-align-left padding" href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/getflowform/{../../flowID}/{flowFormID}" target="_blank">
			...
			<xsl:if test="name">
				<xsl:attribute name="aria-label">
					...
					<xsl:value-of select="$i18n.DownloadFlowFormDescription.Part2" />
					<xsl:text>&#160;</xsl:text>
					<xsl:value-of select="fileExtension" />-<xsl:value-of select="$i18n.DownloadFlowFormDescription.Part3" />
			...
			<img class="alignmiddle marginright" alt="" aria-hidden="true" >
