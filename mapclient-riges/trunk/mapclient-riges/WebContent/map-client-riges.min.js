...
}var a=new Array(mapClientScriptLocation+version+"/lib/ext-theme-oep/ext-theme-oep-all.css",mapClientScriptLocation+version+"/resources/css/OpenEMap.css",mapClientScriptLocation+"/css/map-client-riges.css",mapClientScriptLocation+version+"/lib/Openlayers/theme/default/style.css");var b=new Array(mapClientScriptLocation+version+"/lib/ext/ext-all.js",mapClientScriptLocation+version+"/lib/ext/ext-theme-neptune.js",mapClientScriptLocation+version+"/lib/ext/locale/ext-lang-sv_SE.js",mapClientScriptLocation+version+"/lib/Openlayers/OpenLayers.js",mapClientScriptLocation+version+"/lib/proj4js/proj4-compressed.js",mapClientScriptLocation+version+"/proj4_defs.js",mapClientScriptLocation+version+"/lib/geoext/geoext-all.js",mapClientScriptLocation+version+"/lib/es5-shim/es5-shim.min.js",mapClientScriptLocation+version+"/OpenEMap-min.js");
LoadFiles(a,function(){loadScripts(b)});$("html").addClass("map-client-riges")}})();function loadScripts(c){var e=new Array(c.length);for(var d=0;d<c.length;d++){e[d]="<script src='"+c[d]+"' type='text/javascript'><\/script>"}if(e.length>0){var b=$("head");for(var d=0,a=e.length;d<a;d++){b.append(e[d])}}}function MapClientInstance(queryID,mapID){this.MAP_RESOLUTIONS=[280,140,70,28,14,7,4.2,2.8,1.4,0.56,0.28,0.14,0.112,0.056];this.mapID=mapID;this.mapDiv=$("#"+mapID);this.map;this.featureDialogs={};
this.dialog;this.preview;this.externalGraphicsLocation=mapClientScriptLocation+"images";this.mapImageLocation=mapClientScriptLocation+version+"/resources/images/";this.config;this.originalTools;this.layerMapping={};this.enableEdgeLabeling=false;var instance=this;mapInstances[queryID]=instance;instance.setCustomConfig=function(config,customConfig,preview){if(customConfig.extent){config.extent=customConfig.extent}if(customConfig.search&&config.gui.searchFastighet){config.gui.searchFastighet={renderTo:customConfig.search.renderTo};
...
})}if(instance.config.gui.objectConfig&&instance.config.gui.objectConfig.renderTo){instance.mapDiv.on("click",function(e){var $configWrapper=$("#"+instance.config.gui.objectConfig.renderTo);var $header=$configWrapper.find("span.header");if($header.length==0){$configWrapper.prepend("<span class='header'>"+mapClientLanguages.DIMENSION_AND_ANGLE_SETTINGS+"</span>")}else{$header.text(mapClientLanguages.DIMENSION_AND_ANGLE_SETTINGS)}if($configWrapper.find(".x-panel-default").css("display")=="none"){$configWrapper.css("visibility","hidden")
}else{$configWrapper.css("visibility","visible")}});instance.mapDiv.trigger("click")}var $oepToolBar=$(".oep-tools");$oepToolBar.css("width",($oepToolBar.width()+3)+"px")};instance.zoomToScale=function(scale){instance.map.gui.mapPanel.map.zoomToScale(scale)};instance.showValidationError=function(title,message){$("#query_"+queryID).find("article").addClass("error").addClass("jserror").before('<div class="info-box first error jserror"><span><strong data-icon-before="!">'+title+". </strong>"+message+'</span><div class="marker"></div></div>');
instance.map.gui.mapPanel.map.updateSize()};instance.removeValidationErrors=function(){$("#query_"+queryID).find("article.jserror").removeClass("error").removeClass("jserror").prev(".jserror").remove();instance.map.gui.mapPanel.map.updateSize()};instance.showFeatureDialog=function(feature,layer,message,contentSize,showCloseButton){var $message=null;if(message.jquery){$message=message}else{$message=$(message)}var popup=new OpenLayers.Popup.Anchored("map-client-messagebox_"+mapID,feature.geometry.getBounds().getCenterLonLat(),contentSize,$message.html()+"<div class='marker' />",null,showCloseButton);
popup.displayClass="map-client-messagebox";popup.autoSize=false;var featureHeight=feature.geometry.getBounds().getHeight();var layerStyles=layer.styleMap.styles["default"];if(featureHeight==0){if(layerStyles.defaultStyle.pointRadius=="${getSize}"){featureHeight=layerStyles.context.getSize(feature)}else{if(layerStyles.defaultStyle.pointRadius){featureHeight=layerStyles.defaultStyle.pointRadius}}}var anchor={size:new OpenLayers.Size(0,0),offset:new OpenLayers.Pixel(-188,(featureHeight/2)*-1)};popup.anchor=anchor;
popup.panMapIfOutOfView=true;popup.relativePosition="tr";popup.calculateRelativePosition=function(){return"tr"};feature.popup=popup;instance.map.gui.mapPanel.map.addPopup(popup);instance.featureDialogs[feature.id]=popup};instance.removeFeatureDialog=function(feature){var popup=instance.featureDialogs[feature.id];if(popup!=null){instance.map.gui.mapPanel.map.removePopup(popup);popup=null}};instance.showDialog=function(message,showCloseButton,closeButtonCallback){var $message=null;if(message.jquery){$message=message.clone();
$message.show()}else{$message=$(message)}var $dialog=$("<div class='mapquerydialog olPopup'><div class='olPopupContent'></div></div>");var $content=$dialog.find(".olPopupContent");$content.append($message);if(showCloseButton){var $closeBox=$("<div class='olPopupCloseBox' />");$closeBox.click(function(e){$dialog.close(e)});$content.append($closeBox)}$dialog.close=function(e){if(closeButtonCallback){closeButtonCallback($dialog)}$dialog.remove();Ext.get(instance.mapID).unmask()};Ext.get(instance.mapID).mask();
instance.mapDiv.append($dialog);$dialog.css({left:(instance.mapDiv.width()/2)-($dialog.width()/2),top:(instance.mapDiv.height()/2)-($dialog.height()/2)});instance.dialog=$dialog};instance.getVisibleLayers=function(){var visibleLayers=[];var layers=instance.map.gui.mapPanel.map.layers;for(var i=0;i<layers.length;i++){var layer=layers[i];if(layer.visibility==true){visibleLayers.push(layer)}}return visibleLayers};instance.getVisibleBaseLayers=function(){var visibleBaseLayers=[];var layers=instance.map.gui.mapPanel.map.layers;
for(var i=0;i<layers.length;i++){var layer=layers[i];if(layer.isBaseLayer&&layer.visibility==true){visibleBaseLayers.push(layer)}}return visibleBaseLayers};instance.mapMovedCallback=function(e,instance){};instance.featureAddedCallback=function(e,instance){};instance.featureRemovedCallback=function(e,instance){};instance.beforeFeatureAddedCallback=function(e,instance){};instance.sketchCompletedCallback=function(e,instance){};instance.mapLoadedEventCallback=function(object,instance){};instance.layerVisibilityChangedCallback=function(e,instance){};
instance.baseLayerChangedCallback=function(e,instance){};return instance}function waitUntilMapIsCreated(b,c,a,d){if(!(typeof OpenEMap==="undefined")&&!(typeof OpenEMap.Client==="undefined")){if(a){b.setCustomConfig(c,a,d)}b.initMapClient(c)}else{setTimeout(function(){waitUntilMapIsCreated(b,c,a,d)},5)}};