					...
					<table class="oep-table errand-table">
						<thead class="sortable">
							<tr>
								...
								<th><span data-icon-after="_"><xsl:value-of select="$i18n.ActivityProgress.result" /></span></th>
								<th class="no-sort"></th>
							</tr>
						</thead>
						<tbody>
	...
	<xsl:template match="ActivityProgress" mode="list">
		...
		<tr class="activity">
			...
			<xsl:if test="$showCompleted = 'true'">
				...
				<td data-title="{$i18n.ActivityProgress.result}">
					<xsl:choose>
						<xsl:when test="../../../../useApproveDeny = 'false'">
							<xsl:value-of select="$i18n.ActivityProgress.complete"/>
						</xsl:when>
						
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="denied = 'false'">
