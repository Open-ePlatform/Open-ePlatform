$(function() {
	...
	if ($activityForm.length > 0) {
		...
		const globalNotificationInput = $activityForm[0].querySelector("#onlyUseGlobalNotifications");
		const globalNotificationInfoText = $activityForm[0].querySelector("#onlyUseGlobalNotificationsInfo");

		if (globalNotificationInput && globalNotificationInfoText) {
			globalNotificationInput.addEventListener("click", () => {
				globalNotificationInfoText.classList.toggle("hidden");
			});
		}
