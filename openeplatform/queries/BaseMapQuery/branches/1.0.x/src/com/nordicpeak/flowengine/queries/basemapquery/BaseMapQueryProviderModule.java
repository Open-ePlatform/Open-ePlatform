...
public abstract class BaseMapQueryProviderModule<MapQueryType extends BaseMapQuery, MapQueryInstanceType extends BaseMapQueryInstance<MapQueryType>> extends BaseQueryProviderModule<MapQueryInstanceType> implements BaseQueryCRUDCallback, SiteProfileSettingProvider {
	...
	protected List<PDFAttachment> getPDFAttachments(MapQueryInstanceType queryInstance) {
		...
		if (queryInstance.getFirstMapImage() != null) {
			...
			attachments.add(new BlobPDFAttachment(queryInstance.getFirstMapImage(), this.pdfAttachmentFilename.replace("$scale", this.firstMapImageScale + "") + "." + firstMapImageOutputFormat, this.pdfAttachmentDescriptionPrefix + " " + queryInstance.getQueryInstanceDescriptor().getQueryDescriptor().getName()));

		}

		if (queryInstance.getSecondMapImage()!= null) {

			attachments.add(new BlobPDFAttachment(queryInstance.getSecondMapImage(), this.pdfAttachmentFilename.replace("$scale", this.secondMapImageScale + "") + "." + secondMapImageOutputFormat, this.pdfAttachmentDescriptionPrefix + " " + queryInstance.getQueryInstanceDescriptor().getQueryDescriptor().getName()));

		}

		if (queryInstance.getThirdMapImage() != null) {

			attachments.add(new BlobPDFAttachment(queryInstance.getThirdMapImage(), this.pdfAttachmentFilename.replace("$scale", this.thirdMapImageScale + "") + "." + thirdMapImageOutputFormat, this.pdfAttachmentDescriptionPrefix + " " + queryInstance.getQueryInstanceDescriptor().getQueryDescriptor().getName()));
