...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="scripts">
		/js/childqueryfilterendpoint.js
	</xsl:variable>
	...
	<xsl:template match="ShowChildQueryFilterEndpoint">
		...
		<h1 class="clearfix">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<a class="floatright marginleft" href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/delete/{endpointID}" onclick="return confirmHyperlinkPost(this);" title="{$i18n.DeleteConfirm}: {ChildQueryFilterEndpoint/name}">
			...
			<table class="coloredtable sortabletable oep-table">
				<thead class="sortable">
					<tr>
						...
						<th><xsl:value-of select="$i18n.FlowFamilyID" /></th>
				...
				<tbody>
					<xsl:choose>
						...
						<xsl:otherwise>
							<tr>
								<td colspan="4"><xsl:value-of select="$i18n.NoQueries"/></td>
							</tr>
						</xsl:otherwise>
					</xsl:choose>
				</tbody>
	...
	<xsl:template match="Flow">
		...
		<tr>
			...
			<td><xsl:value-of select="FlowFamily/flowFamilyID"/></td>
			<td>
		...
		<form id="remoteInfoQueryEndpointForm" name="remoteInfoQueryEndpointForm" method="post" action="{/Document/requestinfo/uri}">
			...
			<div class="floatleft margintop full">
				...
				<table class="full border">
					...
					<tr>
						<td>
							<xsl:text>$unfilteredChildCitizenIdentifiers</xsl:text>
						</td>
						<td>
							<xsl:value-of select="$i18n.UnfilteredChildCitizenIdentifiersTag"/>
						</td>
					</tr>
				</table>
			</div>
			
			<xsl:if test="HasTagSupport">
				<div class="d-flex flex-column full">
					<div>
						...
						<xsl:call-template name="createAutocompleteInput">
							<xsl:with-param name="id" select="'tagAutocomplete'"/>
							<xsl:with-param name="class" select="''"/>
							<xsl:with-param name="aria-controls" select="'tags'"/>
							<xsl:with-param name="noResultsText" select="$i18n.NoTagsFound"/>
						</xsl:call-template>
					</div>
					
					<div data-url="{/Document/requestinfo/currentURI}/{/Document/module/alias}/getavailabletags">
						<xsl:call-template name="createTags">
							<xsl:with-param name="id" select="'tags'"/>
							<xsl:with-param name="class" select="'bigmargintop'"/>
							<xsl:with-param name="element" select="SelectedTags/APISourceTag"/>
							<xsl:with-param name="tagValueElementName" select="'tagID'"/>
							<xsl:with-param name="tagLabelElementName" select="'value'"/>
						</xsl:call-template>
					</div>
				</div>
			</xsl:if>
			...
			<xsl:choose>
				<xsl:when test="UseAPISourceAccounts = 'true'">
					<div class="floatleft full">
						<label for="apiSourceAccountID" class="floatleft clearboth">
							<xsl:value-of select="$i18n.ApiSourceAccount"/>
						</label>
						
						<div class="floatleft full">
							<xsl:call-template name="createDropdown">
								<xsl:with-param name="id" select="'apiSourceAccountID'"/>
								<xsl:with-param name="name" select="'apiSourceAccountID'"/>
								<xsl:with-param name="valueElementName" select="'accountID'" />
								<xsl:with-param name="labelElementName" select="'name'" />
								<xsl:with-param name="element" select="APISourceAccountDescriptors/AccountDescriptor"/>
								<xsl:with-param name="addEmptyOption" select="$i18n.ChooseAccount" />
								<xsl:with-param name="selectedValue" select="ChildQueryFilterEndpoint/apiSourceAccountID" />
							</xsl:call-template>
						</div>
					</div>
				</xsl:when>
				<xsl:otherwise>
					...
					<div class="floatleft full bigmarginbottom">
						...
						<label for="username" class="floatleft"><xsl:value-of select="$i18n.Username" /></label>
						<div class="floatleft full">
							<xsl:call-template name="createTextField">
					...
					<div class="floatleft full">
						<label for="password" class="floatleft"><xsl:value-of select="$i18n.Password" /></label>
						<div class="floatleft full">
							<xsl:call-template name="createTextField">
