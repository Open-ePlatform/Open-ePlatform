...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="exsl">
	...
	<xsl:template match="UpdateChildQuery">
		...
		<form id="updateChildQueryForm" name="queryAdminForm" method="post" action="{/Document/requestinfo/uri}">
			...
			<div class="d-flex flex-column full bigmarginbottom">
			
				<label id="calculateAgeTitle" class="full required">
					<xsl:value-of select="$i18n.CalculateMode.Title" />
				</label>
				
				<div id="calculateAgeByYearDesc" class="marginbottom">
					<xsl:value-of select="$i18n.CalculateMode.Description" />
				</div>
				
				<div class="d-flex margintop">
					<xsl:call-template name="createRadio">
						<xsl:with-param name="id" select="'calculateAgeByYear'"/>
						<xsl:with-param name="name" select="'calculateMode'"/>
						<xsl:with-param name="element" select="ChildQuery"/>
						<xsl:with-param name="value" select="'BY_YEAR'"/>
						<xsl:with-param name="aria-describedby" select="'calculateAgeTitle calculateAgeByYearDesc'"/>
					</xsl:call-template>
					
					<label for="calculateAgeByYear" class="full nomargin" >
						<xsl:value-of select="$i18n.CalculateMode.BY_YEAR" />
					</label>
				</div>
				
				<div class="d-flex margintop">
					<xsl:call-template name="createRadio">
						<xsl:with-param name="id" select="'calculateAgeByDate'"/>
						<xsl:with-param name="name" select="'calculateMode'"/>
						<xsl:with-param name="element" select="ChildQuery"/>
						<xsl:with-param name="value" select="'BY_DATE'"/>
						<xsl:with-param name="aria-describedby" select="'calculateAgeTitle calculateAgeByYearDesc'"/>
					</xsl:call-template>
					
					<label for="calculateAgeByDate" class="full nomargin">
						<xsl:value-of select="$i18n.CalculateMode.BY_DATE" />
					</label>
				</div>
			</div>
			...
			<div class="floatleft full bigmarginbottom">
				...
				<p id="setMultipartsAsOwnersSameAddressWarning" class="tiny" style="padding-left: 22px;">
					<xsl:value-of select="$i18n.Query.setMultipartsAsOwnersSameAddress.warning" />
				</p>
			...
			<div id="attributeContainer">
				...
				<table class="floatleft clearboth border bigmarginbottom">
					...
					<tbody>
						...
						<tr>
							<td>prefix.childAddressObjectIdentity</td>
							<td><xsl:value-of select="$i18n.exportChildAddressUUID" /></td>
						</tr>
				...
				<div class="floatleft full bigmarginbottom">
					...
					<table class="floatleft clearboth border">
						...
						<tbody>
							...
							<tr>
								<td>prefix.guardianAddressObjectIdentity</td>
								<td><xsl:value-of select="$i18n.exportOtherGuardianAddressUUID" /></td>
							</tr>
	...
	<xsl:template match="fieldName">
		...
		<xsl:choose>
			...
			<xsl:when test="$fieldName = 'calculateMode'">
				<xsl:value-of select="$i18n.CalculateMode.Title" />
			</xsl:when>
			<xsl:otherwise>
