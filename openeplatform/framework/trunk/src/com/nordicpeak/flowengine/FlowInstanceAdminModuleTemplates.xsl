...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
									...
									<div class="full">
										...
										<div class="upload clearboth">
											<span class="btn btn-upload btn-blue">
												...
												<input id="external-message" type="file" name="attachments" aria-label="{$i18n.ChooseFiles}" aria-describedby="fileUploadInfoNewMessage" multiple="multiple" size="55" class="qloader externalmessages bigmarginbottom" />
											</span>
	  						...
	  						<form action="{/Document/requestinfo/uri}?internalmessage" method="post" enctype="multipart/form-data">
								...
								<div class="full">
									...
									<div class="upload clearboth">
										<span class="btn btn-upload btn-blue">
											<xsl:value-of select="$i18n.ChooseFiles" />
											<input id="internal-message" type="file" name="attachments" multiple="multiple" size="55" aria-label="{$i18n.ChooseFiles}" aria-describedby="fileUploadInfoInternalMessage" class="qloader internalmessages bigmarginbottom" />
										</span>
										<span id="fileUploadInfoInternalMessage"><xsl:value-of select="$i18n.MaximumFileSize" />: <xsl:value-of select="../FormattedMaxFileSize" /></span>
	...
	<xsl:template match="UpdateInstanceManagers">
		...
		<section class="child">
			...
			<form id="newManagersForm" method="post" action="{/Document/requestinfo/uri}">
				...
				<div class="section-full">
					...
					<div class="full bigmarginbottom user-group-list-container" data-search-count="{AvailableManagersCount}" data-url="{/Document/requestinfo/currentURI}/{/Document/module/alias}" data-flowinstanceid="{FlowInstance/flowInstanceID}">
					
						<label for="managerField-input">
						...
						<xsl:call-template name="UserList">
							...
							<xsl:with-param name="hideSearchField" select="true()"/>
						</xsl:call-template>
						
						<div id="loadingManagersText" class="margintop"><xsl:value-of select="$i18n.Search.loadingManagersText"/></div>
						
						<xsl:call-template name="createAutocompleteInput">
							<xsl:with-param name="id" select="'managerField'"/>
							<xsl:with-param name="class" select="''"/>
							<xsl:with-param name="role" select="'combobox'"/>
							<xsl:with-param name="noResultsText" select="$i18n.Search.ManagersNotFound"/>
						</xsl:call-template>
						
						<div id="managerSearchInfolabel" class="hidden">
							<xsl:value-of select="$i18n.Search.ManagerSeachInfo"/>
						</div>
