...
$(function() {
	...
	if ($("#flowForm").length > 0) { // Add/Update flow
		...
		$("#hideInternalMessages").on("change", function() {
			
			const checked = $(this).prop("checked");
			const $allowDeletingInternalMessages = $("#allowDeletingInternalMessages");
			
			if (!checked) {
				$allowDeletingInternalMessages.parent().parent().show();
				$allowDeletingInternalMessages.prop("disabled", false);
			} else {
				$allowDeletingInternalMessages.parent().parent().hide();
				$allowDeletingInternalMessages.prop("disabled", true);
			}
	...
	$("#checkall").on("click", function() {
		...
		if($(this).prop("checked")) {
			$("input[type='checkbox'][name='queryType']").prop("checked", "checked");
		} else {
...
function updateManagerGroupShowHideRowExtra(row) {
	...
	row.find(".hideGroupInAssignment").toggle(row.find("input[name='manager-group-hideGroupInAssignment" + groupID + "']").val() == "true");
...
//						console.info("load " + input.prop("name") + " " + savedInput.val()); 
			...
			afterContent: function() {
				...
				if (row.hasClass("new_row")) {
					...
					content.find('#updateAutoManagerModalCloseButtonContainer').children().toggle();
				}
	...
	var config = {
			...
			afterContent: function() {
				...
				if (row.hasClass("new_row")) {
					...
					content.find('#updateAutoManagerStatusModalCloseButtonContainer').children().toggle();
				}
