$(function() {
	...
	$('.sortMessages').on("click", function(){
		...
		if (isReversed) {
			...
			Cookies.set('reversedMessages.' + this.dataset.type, "false", { path: '/' });
		}
	});

	if (!Cookies.get("hasGlobalMessageCookies")) {
		Cookies.remove("reversedMessages.external", { path: "" });
		Cookies.remove("reversedMessages.internal", { path: "" });

		Cookies.set("hasGlobalMessageCookies", "true", { path: '/' });
	}
	
	if (!Cookies.get("reversedMessages.external") && $(".messages").data("default-message-order") === "reversed") {
		Cookies.set("reversedMessages.external", "true", { expires: 365, path: "/" });
	}
