...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:exsl="http://exslt.org/common" exclude-result-prefixes="exsl">
	...
	<xsl:variable name="scripts">
		...
		/js/flowadminmodule-messagetemplate.js
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<div class="contentitem errands-wrapper flowadmin">
					...
					<xsl:choose>
						<xsl:when test="FlowInstanceManagerForm or FlowInstanceManagerPreview or FlowInstanceManagerSubmitted or ShowFlowOverview or FlowInstanceManagerAllStepsForm">
	...
	<xsl:template match="ShowFlowMenu">
		...
		<section id="flowMenu" class="nomargin">
			...
			<ul class="list-table">
				...
				<li><a href="#managementinfo" title="{$i18n.managementInfo}"><xsl:value-of select="$i18n.managementInfo" /></a></li>
...
<!-- 						TODO: When $i18n.MayNotAddFlowFormIfOverviewSkipIsSet is shown  $i18n.hasNoFlowForm doesn't have to shown. Fix when AllowSkipOverviewForFlowForms has been removed. -->
	...
	<xsl:template match="FlowFamilyManagerGroup" mode="list">
		...
		<div class="floatleft full marginbottom border">
			...
			<xsl:if test="restricted = 'true'">
				...
				<xsl:if test="hideGroupInAssignment = 'true'">
					
					<span class="bigmarginleft">
						<xsl:value-of select="$i18n.Manager.hideGroupInAssignment"/>
					</span>
	...
	<xsl:template match="MessageTemplate" mode="list">
		...
		<tr>
			...
			<td>
				<xsl:choose>
					<xsl:when test="isFormatted = 'true'">
						<xsl:value-of select="$i18n.MessageTemplate.email"/>
					</xsl:when>
	...
	<xsl:template name="flowForm">
		...
		<fieldset>
				...
				<div class="floatleft">
					...
					<div class="floatleft clearboth">
						<xsl:call-template name="createTextField">
							...
							<xsl:with-param name="aria-describedby">publish-dates-info</xsl:with-param>
						</xsl:call-template>
					</div>
				</div>
			...
			<div id="publish-dates-info" class="floatleft full margintop"><xsl:value-of select="$i18n.PublishDatesInfo" /></div>
		...
		<xsl:if test="$isInternal = 'true'">
			...
			<fieldset class="internal">
				...
				<div class="floatleft full bigmarginbottom margintop">
					...
					<div class="floatleft">
						<xsl:call-template name="createCheckbox">
							<xsl:with-param name="name" select="'allowDeletingInternalMessages'" />
							<xsl:with-param name="id" select="'allowDeletingInternalMessages'" />
							<xsl:with-param name="element" select="Flow" />
						</xsl:call-template>
						
						<label for="allowDeletingInternalMessages">
							<xsl:value-of select="$i18n.allowDeletingInternalMessages" />
						</label>
					</div>
				</div>
			...
			<fieldset class="internal">
				...
				<div class="d-flex full bigmarginbottom margintop internal">
					...
					<div class="d-flex bigmargintop">
						<xsl:call-template name="createCheckbox">
							<xsl:with-param name="name" select="'hideOverviewPDF'" />
							<xsl:with-param name="id" select="'hideOverviewPDF'" />
							<xsl:with-param name="element" select="Flow" />
						</xsl:call-template>
						
						<label for="hideOverviewPDF" class="nomargin">
							<xsl:value-of select="$i18n.Flow.hideOverviewPDF" />
						</label>
					</div>
				</div>
				...
				<div class="d-flex full bigmarginbottom margintop internal">
					...
					<div class="d-flex bigmargintop">
						<xsl:call-template name="createCheckbox">
							<xsl:with-param name="name" select="'hideOverviewFlowInstance'" />
							<xsl:with-param name="id" select="'hideOverviewFlowInstance'" />
							<xsl:with-param name="element" select="Flow" />
						</xsl:call-template>
						
						<label for="hideOverviewFlowInstance" class="nomargin">
							<xsl:value-of select="$i18n.Flow.hideOverviewFlowInstance" />
						</label>
					</div>
				</div>
		...
		<fieldset>
			...
			<xsl:if test="$isInternal = 'true'">
				...
				<div class="floatleft">
					<xsl:call-template name="createCheckbox">
						<xsl:with-param name="name" select="'hideAsteriskInfo'" />
						<xsl:with-param name="id" select="'hideAsteriskInfo'" />
						<xsl:with-param name="element" select="Flow" />
					</xsl:call-template>
					
					<label for="hideAsteriskInfo">
						<xsl:value-of select="$i18n.HideAsteriskInfo" />
					</label>
				</div>
	...
	<xsl:template match="AddQueryDescriptor">
		...
		<form method="post" action="{/Document/requestinfo/uri}">
			...
			<div class="floatleft full bigmarginbottom">
				...
				<label for="name" class="floatleft full required">
					<xsl:value-of select="$i18n.name" />
				</label>
			...
			<h2 class="required"><xsl:value-of select="$i18n.defaultQueryState.title"/></h2>
	...
	<xsl:template name="statusForm">
		...
		<div class="floatleft full bigmarginbottom">
			...
			<label for="description" class="floatleft full">
				<xsl:value-of select="$i18n.externalDescription" />
			</label>
		...
		<div class="floatleft full bigmarginbottom">
			...
			<label for="internalDescription" class="floatleft full">
				<xsl:value-of select="$i18n.internalDescription" />
			</label>
			
			<div class="floatleft full">
				<xsl:call-template name="createTextArea">
					<xsl:with-param name="id" select="'internalDescription'"/>
					<xsl:with-param name="name" select="'internalDescription'"/>
					<xsl:with-param name="element" select="$element" />          
				</xsl:call-template>
			</div>
		</div>
	...
	<xsl:template name="flowTypeForm">
		...
		<fieldset class="floatleft full bigmarginbottom">
			...
			<div class="floatleft full margintop">
				...
				<label for="allowAnonymousAccess">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="floatleft full bigmarginbottom">
							...
							<div class="floatleft full bigmarginbottom">
								<input type="checkbox" id="hideGroupInAssignment" name="hideGroupInAssignment" value="true" />
								<label class="marginleft" for="hideGroupInAssignment">
									<xsl:value-of select="$i18n.UpdateManagers.Modal.hideGroupInAssignment" />
								</label>
							</div>
							<div class="floatleft full bigmarginbottom">
	...
	<xsl:template name="grouplist-extension-buttons">
		...
		<xsl:choose>
			<xsl:when test="$listname = 'manager-group'">
				...
				<span class="bigmarginleft hideGroupInAssignment" style="display: none;">
					<xsl:value-of select="$i18n.Manager.hideGroupInAssignment" />
				</span>
	...
	<xsl:template name="grouplist-extension-defaults">
		...
		<xsl:choose>
			<xsl:when test="$listname = 'manager-group'">
				...
				<xsl:call-template name="grouplist-extension-default">
					<xsl:with-param name="listname" select="$listname" />
					<xsl:with-param name="name" select="'hideGroupInAssignment'" />
					<xsl:with-param name="value" select="''" />
				</xsl:call-template>
		...
		<xsl:choose>
			<xsl:when test="$listname = 'manager-group'">
				...
				<xsl:call-template name="grouplist-extension">
					...
					<xsl:with-param name="requestparameters" select="$requestparameters" />
					<xsl:with-param name="name" select="'hideGroupInAssignment'" />
					<xsl:with-param name="value" select="../hideGroupInAssignment" />
				</xsl:call-template>
				...
				<div class="auto-manager-modal contentitem">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div id="updateAutoManagerModalCloseButtonContainer">
								<div class="floatleft full bigmargintop bigmarginbottom hidden">
									<p class="bold"><xsl:value-of select="$i18n.AutoManagerAssignment.Rules.Add.Info" /></p>
								</div>
								<div class="floatleft full bigmargintop bigmarginbottom">
									<p class="bold"><xsl:value-of select="$i18n.AutoManagerAssignment.Rules.Update.Info" /></p>
								</div>
							
								<input class="close bigmargintop floatright hidden" type="button" value="{$i18n.AutoManagerAssignment.Rule.Add}" />
								<input class="close bigmargintop floatright" type="button" value="{$i18n.AutoManagerAssignment.Rule.Update}" />
							</div>
				...
				<div class="auto-manager-status-modal contentitem">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div id="updateAutoManagerStatusModalCloseButtonContainer">
								<div class="floatleft full bigmargintop bigmarginbottom hidden">
									<p class="bold"><xsl:value-of select="$i18n.AutoManagerAssignment.Rules.Add.Info" /></p>
								</div>
								<div class="floatleft full bigmargintop bigmarginbottom">
			...
			<td class="auto-manager-values">
				<xsl:choose>
						...
						<xsl:otherwise>
							...
							<xsl:for-each select="Values/AutoManagerAssignmentRuleAttributeValue">
	...
	<xsl:template match="AddMessageTemplate">
		...
		<form class="messageTemplateForm" method="post" action="{/Document/requestinfo/uri}">
	...
	<xsl:template name="messageTemplateForm">
		...
		<xsl:variable name="isFormattedEmail" select="MessageTemplate/isFormatted = 'true'"/>
		...
		<div>
				...
				<div class="floatleft full">
					<xsl:call-template name="createRadio">
						...
						<xsl:with-param name="checked" >
							<xsl:if test="MessageTemplate/isFormatted = 'false'">
								<xsl:value-of select="MessageTemplate/type = 'EXTERNAL'"/>
							</xsl:if>
						</xsl:with-param>
					</xsl:call-template>
			...
			<div class="floatleft full bigmarginbottom">
				...
				<div class="floatleft full">
					<xsl:call-template name="createRadio">
						<xsl:with-param name="id" select="'messageTemplateEmail'"/>
						<xsl:with-param name="name" select="'messageIsFormatted'"/>
						<xsl:with-param name="value" select="'messageIsFormatted'"/>
						<xsl:with-param name="checked" select="$isFormattedEmail" />
					</xsl:call-template>
					
					<label for="messageTemplateEmail">
						<xsl:value-of select="$i18n.MessageTemplate.email" />
					</label>
				</div>
			</div>	
		...
		<div class="floatleft full bigmarginbottom">
			<label class="floatleft full">
				<xsl:attribute name="for">
					<xsl:choose>
						<xsl:when test="$isFormattedEmail">
							<xsl:text>messageTemplateMessageCK</xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xsl:text>messageTemplateMessage</xsl:text>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:attribute>
			...
			<div class="floatleft full">
				...
				<div>
					<xsl:if test="not($isFormattedEmail)">
						<xsl:attribute name="class">hidden</xsl:attribute>
					</xsl:if>
					
					<xsl:call-template name="createTextArea">
						<xsl:with-param name="id" select="'messageTemplateMessageCK'"/>
						<xsl:with-param name="name" select="'messageTemplateMessage'"/>
						<xsl:with-param name="disabled" select="not($isFormattedEmail)" />
						<xsl:with-param name="class" select="'flow-ckeditor'"/>
				...
				<div>
					...
					<xsl:call-template name="createTextArea">
						...
						<xsl:with-param name="name" select="'messageTemplateMessage'"/>
						<xsl:with-param name="disabled" select="$isFormattedEmail" />
						<xsl:with-param name="value">
							<xsl:if test="not($isFormattedEmail)">
								<xsl:value-of select="MessageTemplate/message"/>
							</xsl:if>
						</xsl:with-param>
					</xsl:call-template>
				</div>
		...
		<section class="service has-navigator testing">
			...
			<xsl:for-each select="ManagerResponses/ManagerResponse">
				...
				<div class="service-navigator-wrap">
					<div>
						<ol class="service-navigator primary full">
