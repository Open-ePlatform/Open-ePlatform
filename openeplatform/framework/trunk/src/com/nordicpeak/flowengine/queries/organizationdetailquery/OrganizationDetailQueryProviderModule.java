...
public class OrganizationDetailQueryProviderModule extends BaseQueryProviderModule<OrganizationDetailQueryInstance> implements BaseQueryCRUDCallback {
	...
	public void populate(OrganizationDetailQueryInstance queryInstance, HttpServletRequest req, User user, User poster, boolean allowPartialPopulation, MutableAttributeHandler attributeHandler, InstanceRequestMetadata requestMetadata) throws ValidationException {
		...
		if(query.isForceSMS()) {
			...
			attributeHandler.setAttribute(Constants.FLOW_INSTANCE_FORCE_SMS, "true");
			
		} else {
			
			attributeHandler.removeAttribute(Constants.FLOW_INSTANCE_FORCE_SMS);
		}
		...
		String qParam = "q" + queryID + "_";
		
		if(query.isRequireEmailOrMobile() && StringUtils.isEmpty(email) && StringUtils.isEmpty(mobilePhone)) {
			
			errors.add(new ValidationError("NoContactChannelChoosen", qParam));							
		}
		
		if(query.isRequireEmailOrMobile() && StringUtils.isEmpty(email) && !StringUtils.isEmpty(mobilePhone) && !contactBySMS && query.isAllowSMS()) {
			
			errors.add(new ValidationError("SMSNotificationNotChosen", qParam + "notification-channel-settings", qParam));
		}
				
		String firstname;
		String lastname;

		String rawEmail = req.getParameter(qParam + "email");
		String rawPhone = req.getParameter(qParam + "phone");
		String rawMobilePhone = req.getParameter(qParam + "mobilephone");
		...
		if (poster != null) {
			...
			firstname = ValidationUtils.validateParameter(qParam + "firstname", req, !allowPartialPopulation, stringPopulator, errors);
			lastname = ValidationUtils.validateParameter(qParam + "lastname", req, !allowPartialPopulation, stringPopulator, errors);
		...
		if(!requireAddressFields && !allowPartialPopulation){
			...
			if (StringUtils.isEmpty(address) && (query.getFieldAddress() == OrganizationDetailQueryField.REQUIRED || (!StringUtils.isEmpty(zipCode) || !StringUtils.isEmpty(postalAddress)))) {
				errors.add(new ValidationError(qParam + "address", ValidationErrorType.RequiredField));
			}
			...
			if (StringUtils.isEmpty(zipCode) && (query.getFieldAddress() == OrganizationDetailQueryField.REQUIRED || (!StringUtils.isEmpty(address) || !StringUtils.isEmpty(postalAddress)))) {
				errors.add(new ValidationError(qParam + "zipcode", ValidationErrorType.RequiredField));
			}
			...
			if (StringUtils.isEmpty(postalAddress) && (query.getFieldAddress() == OrganizationDetailQueryField.REQUIRED || (!StringUtils.isEmpty(address) || !StringUtils.isEmpty(zipCode)))) {
				errors.add(new ValidationError(qParam + "postaladdress", ValidationErrorType.RequiredField));
			}
		}		

		this.validateFieldLength(qParam + "name", name, 255, errors);
		this.validateFieldLength(qParam + "organizationNumber", organizationNumber, 16, errors);
		this.validateFieldLength(qParam + "address", address, 255, errors);
		this.validateFieldLength(qParam + "zipcode", zipCode, 10, errors);
		this.validateFieldLength(qParam + "postaladdress", postalAddress, 255, errors);
		this.validateFieldLength(qParam + "firstname", firstname, 255, errors);
		this.validateFieldLength(qParam + "lastname", lastname, 255, errors);
		this.validateFieldLength(qParam + "mobilephone", mobilePhone, 255, errors);
		this.validateFieldLength(qParam + "email", email, 255, errors);
		this.validateFieldLength(qParam + "phone", phone, 255, errors);		
		
		if (!StringUtils.isEmpty(email)) {
			if (!EmailUtils.isValidEmailAddress(email)) {
				errors.add(new ValidationError(qParam + "email", ValidationErrorType.InvalidFormat));
			}
		}
		
		if (query.isRequireContactInfoVerification()) {
			String confirmEmailID = qParam + "confirmEmail";
			String confirmMobilePhoneID = qParam + "confirmMobilephone";

			String confirmEmail = ValidationUtils.validateParameter(confirmEmailID, req, query.isRequireContactInfoVerification() && query.getFieldEmail() != OrganizationDetailQueryField.HIDDEN && !allowPartialPopulation && !StringUtils.isEmpty(email), 0, 255, EmailPopulator.getPopulator(), errors);
			String confirmMobilePhone = ValidationUtils.validateParameter(confirmMobilePhoneID, req, query.isRequireContactInfoVerification() && query.getFieldMobilePhone() != OrganizationDetailQueryField.HIDDEN && !allowPartialPopulation && !StringUtils.isEmpty(mobilePhone), StringSwedishPhoneNumberPopulator.getPopulator(), errors);

			
			if (((email == null && confirmEmail != null) || (email != null && !email.equalsIgnoreCase(confirmEmail))) && !ValidationUtils.containsValidationErrorForField(confirmEmailID, errors)) {

				errors.add(new ValidationError("EmailVerificationMismatch", "", confirmEmailID));
			}

			if (mobilePhone !=null && confirmMobilePhone != null && query.getFieldMobilePhone() != OrganizationDetailQueryField.HIDDEN) {

				// Only keep numbers when comparing, to handle different formats of mobile phone numbers
				String strippedMobilePhone = mobilePhone.replaceAll("[^+0-9]", "");
				String strippedConfirmMobilePhone = confirmMobilePhone.replaceAll("[^+0-9]", "");
				
				if ((mobilePhone != null && !strippedMobilePhone.equals(strippedConfirmMobilePhone)) && !ValidationUtils.containsValidationErrorForField(confirmMobilePhoneID, errors)) {
					errors.add(new ValidationError("MobilePhoneVerificationMismatch", "", confirmMobilePhoneID));
				}
			} else if(!StringUtils.isEmpty(mobilePhone) && confirmMobilePhone != null) {
				errors.add(new ValidationError("MobilePhoneVerificationMismatch", "", confirmMobilePhoneID));
			}		
		}
