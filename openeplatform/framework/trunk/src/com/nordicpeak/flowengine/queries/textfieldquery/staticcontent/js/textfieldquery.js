...
//			$textField.tooltip("destroy");
	...
	resetTextFieldValidationErrors(queryID);

	runQueryEvaluators(queryID, parameters);
	
}

function getTextFieldParameters($query) {
	
	const parameters = {};
	
	$query.find("input[type='text'], input[type='date'], input[type='password']").each(function() {
...
function resetTextFieldValidationErrors(queryID) {
	
	const query = document.querySelector("#query_" + queryID);
	const article = query.querySelector("article");

	article.querySelectorAll(".error").forEach((error) => {
		error.remove();

		article.querySelectorAll("input[aria-invalid='true']").forEach((inputField) => {
...
function setTextFieldValidationError(queryID, validationErrors) {
	...
	const validationErrorTemplateWrapper = query.querySelector(".validationErrorTemplate");
	
	if (validationErrorTemplateWrapper) {
		
		validationErrors.forEach((validationError, i) => {
			const validationErrorTemplate = validationErrorTemplateWrapper.firstElementChild.cloneNode(true);
			
			const fieldID = `q${queryID}_field${validationError.field}`;
			
			const field = query.querySelector("#" + fieldID);
			
			validationErrorTemplate.id = fieldID + "_error";
			validationErrorTemplate.append(validationError.displayName);
			
			field.nextElementSibling.after(validationErrorTemplate);
			
			field.parentElement.classList.add("input-error");
			
			field.setAttribute("aria-invalid", true);
			field.setAttribute("aria-describedby", validationErrorTemplate.id);
		});
