...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
			...
			<article>
				...
				<div class="heading-wrapper">
					...
					<xsl:call-template name="createUpdateButton">
						...
						<xsl:with-param name="queryName" select="FileUploadQueryInstance/QueryInstanceDescriptor/QueryDescriptor/name" />
					</xsl:call-template>
					...
					<xsl:if test="not(Locked)">
						<div class="upload clearboth">
							<span class="btn btn-upload btn-blue">
								...
								<input id="{$queryID}_fileuploader" type="file" name="{concat('q', FileUploadQueryInstance/FileUploadQuery/queryID,'_newfile')}" aria-describedby="fileUploadInfo" multiple="multiple" size="55" class="fileuploader bigmarginbottom hidden" data-allowedfilesregex="{$allowedFileExtensionsRegex}" data-filecountlimit="{$fileCountLimit}" aria-label="{$i18n.ChooseFiles}" />
							</span>
							...
							<div id="fileUploadInfo">								
								...
								<xsl:if test="FileUploadQueryInstance/FileUploadQuery/maxQuerySize">
									<span>
										<xsl:value-of select="$i18n.MaximumQuerySize" /><xsl:text>:&#x20;</xsl:text>
										<xsl:value-of select="FileUploadQueryInstance/FileUploadQuery/FormatedMaxQuerySize" />
									</span>
								</xsl:if>
	...
	<xsl:template match="FileDescriptor">
		...
		<li class="finished">
			<div class="file">
				...
				<xsl:if test="not(../../../Locked)">
					<a data-icon-after="t" href="#" role="button" onclick="removeFile(event, 'q{../../FileUploadQuery/queryID}_file{temporaryFileID}', '{$i18n.ConfirmDeleteFile} {$file-name}?', this)" class="progress"><xsl:value-of select="$i18n.DeleteFile" /></a>
				</xsl:if>
	...
	<xsl:template match="validationError[messageKey='QuerySizeLimitExceeded']">
	
		<span>
			<strong data-icon-before="!">
				<xsl:value-of select="$i18n.QuerySizeLimitExceeded.part1"/>
				<xsl:value-of select="displayName"/>
				<xsl:value-of select="$i18n.QuerySizeLimitExceeded.part2"/>
			</strong>
		</span>
