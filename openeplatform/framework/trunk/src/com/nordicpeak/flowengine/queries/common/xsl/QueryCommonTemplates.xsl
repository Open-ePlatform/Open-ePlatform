...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template name="createUpdateButton">
		...
		<xsl:if test="updateURL">
			<div class="edit">
				<a data-icon-before="w" href="{updateURL}#query_{$queryID}" class="btn btn-light update_query" aria-label="{$i18n.UpdateQueryTitle}: {$queryName}"><xsl:value-of select="$i18n.UpdateQuery" /></a>
			</div>
		</xsl:if>
