	...
	public void populate(ManualMultiSignQueryInstance queryInstance, HttpServletRequest req, User user, User poster, boolean allowPartialPopulation, MutableAttributeHandler attributeHandler, InstanceRequestMetadata requestMetadata) throws ValidationException {
		...
		if (queryInstance.getQueryInstanceDescriptor().getQueryState() == QueryState.VISIBLE_REQUIRED || !StringUtils.isEmpty(req.getParameter("q" + queryID + "_socialSecurityNumber")) || !StringUtils.isEmpty(req.getParameter("q" + queryID + "_firstname")) || !StringUtils.isEmpty(req.getParameter("q" + queryID + "_lastname")) || !StringUtils.isEmpty(req.getParameter("q" + queryID + "_email")) || !StringUtils.isEmpty(req.getParameter("q" + queryID + "_mobilePhone"))) {
			...
			if (!allowPartialPopulation && errors.isEmpty() && email == null && mobilePhone == null) {
				...
				errors.add(new ValidationError("NoContactChannelSpecified", "q" + queryID + "_"));
			}
			
			if (queryInstance.getQuery().isRequireContactInfoVerification()) {
				
				String confirmEmailID = "q" + queryID + "_confirmEmail";
				String confirmMobilePhoneID =  "q" + queryID + "_confirmMobilePhone";
				
				String confirmEmail = ValidationUtils.validateParameter("q" + queryID + "_confirmEmail", req, email != null && !email.isEmpty(), 1, 255, EMAIL_POPULATOR, errors);
				String confirmMobilePhone = ValidationUtils.validateParameter("q" + queryID + "_confirmMobilePhone", req, mobilePhone != null &&  !mobilePhone.isEmpty(), StringSwedishPhoneNumberPopulator.getPopulator(), errors);
				...
				if (mobilePhone != null && confirmMobilePhone != null) {
					...
					if (((mobilePhone == null && confirmMobilePhone != null) || (mobilePhone != null && !strippedMobilePhone.equals(strippedConfirmMobilePhone)) && !ValidationUtils.containsValidationErrorForField(confirmMobilePhoneID, errors))) {
						errors.add(new ValidationError("MobilePhoneVerificationMismatch", "", "q" + queryID + "_confirmMobilePhone"));
					}
				} else if(mobilePhone == null && confirmMobilePhone != null) {
					errors.add(new ValidationError("MobilePhoneVerificationMismatch", "", "q" + queryID + "_confirmMobilePhone"));
				}		
			}
