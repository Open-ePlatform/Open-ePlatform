...
import com.nordicpeak.flowengine.interfaces.ListTodosViewFragmentExtensionProvider;
...
public class UserFlowInstanceModule extends BaseFlowBrowserModule implements MessageCRUDCallback, NotificationSource, UserMenuProvider {
	...
	@ModuleSetting
	@CheckboxSettingDescriptor(name = "Reverse default external message order", description = "Option for reversing the default order of external messages. If true, the newest messages will be shown at the top.")
	protected boolean reverseExternalMessageOrder;
	...
	@ModuleSetting
	@EnumDropDownSettingDescriptor(name = "External message sort order", description = "The order of external messages when displayed in this module", required = true)
	protected Order externalMessageSortOrder = Order.ASC;
	...
	public ForegroundModuleResponse showFlowInstanceOverview(HttpServletRequest req, HttpServletResponse res, User user, URIParser uriParser) throws ModuleConfigurationException, SQLException, AccessDeniedException, IOException {
		...
		if (uriParser.size() == 4 && NumberUtils.isInt(uriParser.get(3)) && (flowInstance = getFlowInstance(Integer.valueOf(uriParser.get(3)), CollectionUtils.getList(ExternalMessageAttachment.DATA_FIELD), FLOW_INSTANCE_OVERVIEW_RELATIONS)) != null) {
			...
			if (externalMessageSortOrder == Order.DESC) {
				
				XMLUtils.appendNewElement(doc, showFlowInstanceOverviewElement, "ReverseExternalMessageOrder");
			}
	...
	protected void appendFlowInstanceOverviewElement(Document doc, Element showFlowInstanceOverviewElement, FlowInstance flowInstance, HttpServletRequest req, HttpServletResponse res, User user, URIParser uriParser, SimpleForegroundModuleResponse moduleResponse) {
		...
		appendFirstAndLastSubmitEvents(genDoc, showFlowInstanceOverviewElement, flowInstance);
	...
	protected void appendFirstAndLastSubmitEvents(Document doc, Element showFlowInstanceOverviewElement, FlowInstance flowInstance) {
		
		if (!CollectionUtils.isEmpty(flowInstance.getEvents())) {
			FlowInstanceEvent firstSubmitEvent = null;
			FlowInstanceEvent lastSubmitEvent = null;
			
			for (FlowInstanceEvent event : flowInstance.getEvents()) {
				
				if (event.getEventType().equals(EventType.SUBMITTED) ){
					if (firstSubmitEvent == null || event.getAdded().before(firstSubmitEvent.getAdded())) {
						firstSubmitEvent = event;
					}
					
					if (lastSubmitEvent == null || event.getAdded().after(lastSubmitEvent.getAdded())) {
						lastSubmitEvent = event;
					}
				}
			}
			
			XMLUtils.append(doc, showFlowInstanceOverviewElement, "FirstSubmitEvent", CollectionUtils.getGenericSingletonList(firstSubmitEvent));
			XMLUtils.append(doc, showFlowInstanceOverviewElement, "LastSubmitEvent", CollectionUtils.getGenericSingletonList(lastSubmitEvent));
		}
	}
	...
	public boolean addListTodosViewFragmentExtensionProvider(ListTodosViewFragmentExtensionProvider provider) {

		return false;
	}

	public boolean removeListTodosViewFragmentExtensionProvider(ListTodosViewFragmentExtensionProvider provider) {
	...
	public FlowInstance deleteFlowInstance(Integer flowInstanceID, FlowInstanceAccessController accessController, User user) throws SQLException, AccessDeniedException {
		...
		query.addRelations(getDeleteRelations());
