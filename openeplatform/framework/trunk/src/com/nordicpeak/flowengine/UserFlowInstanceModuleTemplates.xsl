...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="scripts">
		...
		/js/flowengine.js?v=7
		/js/flowengine.tablesorter.js
		/js/userflowinstancemodule.js?v=2
	...
	<xsl:variable name="links">
		...
		/css/userflowinstancemodule.css?v=1
	</xsl:variable>
			...
			<section class="my-errands">
				...
				<div class="errands-wrapper messages">
						<table class="oep-table errand-table">
							<caption class="screen-reader-only"><xsl:value-of select="$i18n.MyMessages" /></caption>
							<thead>
								<tr>
									<th class="icon"></th>
									<th class="errando">
		...
		<section class="my-errands">
			...
			<xsl:if test="SavedFlowInstances/FlowInstance">
				...
				<div class="errands-wrapper draft">
					...
					<table class="oep-table errand-table">
					<caption class="screen-reader-only"><xsl:value-of select="$i18n.SavedFlowInstancesTitle" /></caption>
						<thead>
							<tr>
								<th class="icon"></th>
			...
			<div class="errands-wrapper">
				...
				<table class="oep-table errand-table">
				<caption class="screen-reader-only"><xsl:value-of select="$i18n.SubmittedFlowInstances" /></caption>
			...
			<div class="errands-wrapper">
				...
				<table class="oep-table errand-table">
					<caption class="screen-reader-only"><xsl:value-of select="$i18n.ArchivedFlowInstances" /></caption>
					<thead class="sortable">
						<tr>
							<th class="icon no-sort"></th>
							<th class="service"><span data-icon-after="_"><xsl:value-of select="$i18n.FlowName" /></span></th>
				...
				<div class="section-inside header-full no-pad-top">
					<div class="description">
						...
						<p>
							...
							<xsl:value-of select="../FirstSubmitEvent/FlowInstanceEvent/added" />
								
							<xsl:if test="../FirstSubmitEvent/FlowInstanceEvent/poster/user/userID != /Document/user/userID">
								<xsl:text>&#160;</xsl:text>
								<xsl:value-of select="$i18n.by" /><xsl:text>&#160;</xsl:text>
								
								<!-- The poster's ID is the value of the hideManagerInfo attribute -->
								<xsl:variable name="isUserPosterAndHideManagerInfoEnabled" select="Attributes and Attributes/Attribute[Name = 'hideManagerInfo'] and Attributes/Attribute[Name = 'hideManagerInfo']/Value = /Document/user/userID"/>
								
								<xsl:choose>
									<xsl:when test="$isUserPosterAndHideManagerInfoEnabled">
										<xsl:value-of select="$i18n.Manager"/>
									</xsl:when>
									
									<xsl:otherwise>
										<xsl:call-template name="PrintPostedBy">
											<xsl:with-param name="poster" select="../FirstSubmitEvent/FlowInstanceEvent/poster/user"/>
						...
						<xsl:if test="count($submittedEvents) > 1">
							<p>
								<xsl:variable name="lastSubmit" select="../LastSubmitEvent/FlowInstanceEvent"/>
					...
					<div id="messages">
						...
						<div id="new-message" class="tabs-content">
							...
							<div class="heading-wrapper">
								...
								<a href="#" class="btn btn-light btn-right close_message"><xsl:value-of select="$i18n.Close" /><i aria-hidden="true" data-icon-after="x"></i></a>
							</div>
								...
								<div class="full">
									...
									<div class="upload clearboth">
										<span class="btn btn-upload btn-blue">
											...
											<input id="external-message" type="file" name="externalmessage-attachments" aria-label="{$i18n.ChooseFiles}" aria-describedby="fileUploadInfoNewMessage" multiple="multiple" size="55" class="qloader externalmessages bigmarginbottom" />
										</span>
										<span id="fileUploadInfoNewMessage"><xsl:value-of select="$i18n.MaximumFileSize" />: <xsl:value-of select="../FormattedMaxFileSize" /></span>
						...
						<div class="tabs-content">
							...
							<xsl:choose>
								...
								<xsl:call-template name="sortMessagesButton" />
									...
									<ul class="messages">
										<xsl:if test="../ReverseExternalMessageOrder">
											<xsl:attribute name="data-default-message-order">reversed</xsl:attribute>
										</xsl:if>
			...
			<xsl:if test="../../Flow/hideManagerDetails = 'false'">
				<td data-title="{$i18n.Details}">
					<xsl:choose>
						...
						<xsl:when test="Attributes/Attribute[Name='externalMessageReadByAllParts']">
							<xsl:variable name="messageID" select="Attributes/Attribute[Name='externalMessageID']/Value"></xsl:variable>
							<a class="messagelink" href="#messages-{$messageID}-openreceipt">
								<xsl:value-of select="$i18n.ReadByAllPartsDetails"></xsl:value-of>
							</a>
						</xsl:when>
	...
	<xsl:template name="createHelpDialog">
		...
		<div class="help {$class}">
			<a class="open-help" href="#" role="button" data-icon-after="?" data-help-box="helpdialog_{$id}"><span><xsl:value-of select="$i18n.Help" /></span></a>
			<div class="help-box" data-help-box="helpdialog_{$id}">
				<div>
						<div> 
							<a class="close" role="button" href="#"><xsl:value-of select="$i18n.Close" /><i aria-hidden="true" data-icon-after="x"></i></a>
