...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="ShowFlowFeedbackSurveys">
		...
		<xsl:variable name="scriptPath"><xsl:value-of select="/Document/StaticContentURL"/>/js</xsl:variable>
		<xsl:variable name="linkPath"><xsl:value-of select="/Document/StaticContentURL"/>/css</xsl:variable>
		
		<div id="FeedbackFlowSubmitSurvey">
			<p class="nomargin"><strong><xsl:value-of select="$i18n.FeedbackSurveyTitle" /></strong></p>
			
			<xsl:if test="isRestrictedFlowAdmin = 'true'">
				<script src="{$scriptPath}/d3.v3.min.js"/>
				<script src="{$scriptPath}/c3.min.js"/>
				<script src="{$scriptPath}/feedbacksurvey.js"/>
				<link rel="stylesheet" href="{$linkPath}/c3.min.css"/> 
				<link rel="stylesheet" href="{$linkPath}/feedbacksurvey.css" /> 
			</xsl:if>
			
			<script>
				i18nFeedbackFlowSubmitSurveyModule = {};
			...
			<div class="showflow-content nomargin nopadding clearboth">
				...
				<xsl:choose>
					<xsl:when test="isRestrictedFlowAdmin = 'true'">
						<a href="{/Document/ContextPath}{extensionRequestURL}/updateflowsettings" class="floatright">
				...
				<xsl:if test="FeedbackSurveySettings">
					...
					<xsl:choose>
						<xsl:when test="isRestrictedFlowAdmin = 'true'">
							<a class="marginleft floatright" href="{/Document/ContextPath}{extensionRequestURL}/deleteflowsettings" onclick="return confirmHyperlinkPost(this);" title="{$i18n.DeleteSettings}">
				...
				<xsl:attribute name="action">
					<xsl:choose>
						<xsl:when test="isRestrictedFlowAdmin = 'true'">
							...
							<xsl:text>/updateflowsettings</xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="/Document/ModuleURI"/>
							<xsl:text>/updatesettings/</xsl:text>
