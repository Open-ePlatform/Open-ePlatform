...
public class MessageHandler {
	...
	public void delete(InternalMessage internalMessage) throws SQLException, IOException {
		
		InternalMessageHandlerInstance messageHandlerInstance = getInstance(internalMessage);
		
		messageHandlerInstance.delete();
	}
	...
	public void commited(MessageHandlerInstance<? extends BaseMessage> messageHandlerInstance) throws SQLException {
		...
		messageHandlerInstance.addCommited();
	}
	...
	public abstract class MessageHandlerInstance<Message extends BaseMessage> {
		...
		private void delete(TransactionHandler transactionHandler) throws SQLException, IOException {

			log.info("User " + message.getPoster() + " deleting " + message.getTypeLogName() + " message for flowinstance " + message.getFlowInstance());
			
			RelationQuery relations = new RelationQuery();
			
			if (message instanceof InternalMessage) {
				relations.addRelation(InternalMessage.ATTACHMENTS_RELATION);
				
				if (!CollectionUtils.isEmpty(message.getAttachments())) {
					Iterator<? extends BaseAttachment> iterator = message.getAttachments().iterator();
					
					while (iterator.hasNext()) {
						BaseAttachment attachment = iterator.next();
						
						FlowEngineFileAttachmentUtils.deleteAttachment(flowAdminModule.getFileAttachmentHandler(), message, attachment);
			...
			messageDAO.update(message, transactionHandler, relations);
		}

		public void addCommited() throws SQLException {
		...
		public void updateCommited() {
			
			flowAdminModule.getSystemInterface().getEventHandler().sendEvent(this.getClass(), new CRUDEvent<>(CRUDAction.UPDATE, message), EventTarget.ALL);
		}
	...
	public class ExternalMessageHandlerInstance extends MessageHandlerInstance<ExternalMessage> {
		...
		public void addCommited() throws SQLException {
			...
			FlowInstanceEvent flowInstanceEvent = flowAdminModule.getFlowInstanceEventGenerator().addFlowInstanceEvent(flowInstance, eventType, null, user, added, ExternalMessageUtils.getFlowInstanceEventAttributes(message, false));
	...
	public class InternalMessageHandlerInstance extends MessageHandlerInstance<InternalMessage> {
		...
		public void addCommited() throws SQLException {
			...
			if(flowAdminModule.isUpdateLastEventAddedForInternalMessages()) {
			
				LowLevelQuery<FlowInstance> lastAddedEventQuery = new LowLevelQuery<>("UPDATE " + flowAdminModule.getDAOFactory().getFlowInstanceDAO().getTableName() + " SET lastEventAdded = ? WHERE flowInstanceID = ?");
	
				lastAddedEventQuery.addParameter(message.getAdded());
				lastAddedEventQuery.addParameter(flowInstance.getFlowInstanceID());
	
				flowAdminModule.getDAOFactory().getFlowInstanceDAO().update(lastAddedEventQuery);
	...
	public void addReadReceipt(ExternalMessageReadReceipt readReceipt, ExternalMessage externalMessage) throws SQLException {
		...
		if (allOwnersHaveReadMessage(flowInstance.getOwners(), allReadReceipts)) {
			...
			flowInstanceEvent = flowAdminModule.getFlowInstanceEventGenerator().addFlowInstanceEvent(flowInstance, EventType.CUSTOMERS_READ_MESSAGE, null, null, added, ExternalMessageUtils.getFlowInstanceEventAttributes(externalMessage, true));
		}
